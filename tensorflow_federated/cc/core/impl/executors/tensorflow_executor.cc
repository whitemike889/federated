...
// If building with dynamicly linked TensorFlow we use the runtime reflection
...
void AddDatasetToGraphOp(tensorflow::GraphDef& graphdef_pb,
  ...
  auto repeated_attrs =
      node_reflection->GetMutableRepeatedFieldRef<::google::protobuf::Message>(
          graph_to_dataset_node.get(),
          node_descriptor->FindFieldByName("attr"));
  // value_reflection->SetInt64(value_msg,
  //                            attr_descriptor->FindFieldByName("i"), 0);
  // map_key.SetStringValue("external_state_policy");
  // // We strip the device placement, we want the session we're about to enter
  // // to provide the device placement.
  // map_key.SetStringValue("strip_device_assignment");
  // value_reflection->SetBool(value_msg,
  //                            attr_descriptor->FindFieldByName("b"), true);
